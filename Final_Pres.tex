...
\begin{frame}[label={sec:orge5e1143}]{Definitions}
\begin{definition}[Iterative Method]\label{sec:orgd5c25da}
A sequence where each value in the sequence is defined by the previous value.
\end{definition}

\begin{definition}<2->[Dynamical System]\label{sec:orgd969951}
A system that enacts rules on a set of variables to produce a state.
\end{definition}

\begin{definition}<3->[Complex Dynamics]\label{sec:orge14012a}
The study of \uline{dynamical systems} defined by complex functions.
\end{definition}
\end{frame}

\section{Iteration}
\label{sec:orga170e5d}

\begin{frame}[label={sec:orga9175b7}]{Iteration}
\begin{definition}[Function Iteration]\label{sec:org5c06c71}
\(f^0 := \symbf{I}\)

\(f^{k+1} := f \circ f^k\)
\end{definition}

\begin{exampleblock}<2->{Example}\label{sec:org186bb65}
Given \(f(x) = x + 1\),
...
\begin{definition}[Complex Numbers]\label{sec:orged8a577}
\(\symbf{i}^2 = -1\)
...
\begin{frame}[label={sec:orga85d5b3}]{Complex Iteration}
\begin{columns}
\begin{column}{0.5\columnwidth}
\begin{block}{Rules}
\(f(z) = z^2\)
...
\begin{frame}[label={sec:org815b27a},b]{Quaternions History}
\begin{columns}
\begin{column}{0.50\columnwidth}
\begin{figure}[htbp]
...
\begin{frame}[label={sec:orgfc527b1}]{Quaternions}
\begin{definition}[Quaternion]\label{sec:orgfdc15f7}
...
\begin{frame}[label={sec:orgb69c11c},b]{Plotting}
\begin{columns}
\begin{column}{0.5\columnwidth}
\begin{figure}[htbp]
\centering
...
\begin{frame}[label={sec:org353d7a1}]{Ray Tracing}
\begin{figure}[htbp]
...
\begin{frame}[label={sec:orgf0325d1}]{Ray Marching}
\begin{columns}
\begin{column}{0.5\columnwidth}
\begin{figure}[htbp]
...
\begin{column}{0.5\columnwidth}
\begin{theorem}[Distance Estimation]\label{sec:orga9a5010}
...
\begin{frame}[label={sec:org53b0fe9}]{Normal Estimation}
\begin{figure}[htbp]
...
\begin{frame}[label={sec:orgd5b5757}]{Quaternion Iterative Fractals}
\begin{columns}
\begin{column}{0.55\columnwidth}
...
\begin{frame}[label={sec:orgd93e6ea}]{Summary}
\setcounter{tocdepth}{2}
\tableofcontents
\end{frame}
...
\begin{frame}[label={sec:org6b8f52d}]{Questions?}
\begin{center}
